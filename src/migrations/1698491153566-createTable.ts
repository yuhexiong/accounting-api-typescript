...
export class CreateTable1698491153566 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`CREATE TABLE \`type\` (\`id\` varchar(20) NOT NULL COMMENT 'id', \`createDate\` datetime(6) NOT NULL COMMENT '修改時間' DEFAULT CURRENT_TIMESTAMP(6), \`name\` varchar(20) NOT NULL COMMENT '名稱', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    await queryRunner.query(`CREATE TABLE \`consumption\` (\`id\` bigint NOT NULL AUTO_INCREMENT COMMENT '流水號', \`date\` varchar(20) NOT NULL COMMENT '日期', \`typeId\` varchar(100) NOT NULL COMMENT '類別', \`name\` varchar(100) NOT NULL COMMENT '名稱', \`amount\` bigint NOT NULL COMMENT '金額' DEFAULT '0', \`note\` varchar(200) NULL COMMENT '名稱', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    await queryRunner.query(`CREATE TABLE \`report\` (\`id\` bigint NOT NULL AUTO_INCREMENT COMMENT '流水號', \`year\` smallint NOT NULL COMMENT '年', \`month\` smallint NOT NULL COMMENT '月', \`content\` json NOT NULL COMMENT '依分類金額' DEFAULT '{}', \`totalAmount\` bigint NOT NULL COMMENT '總金額' DEFAULT '0', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
